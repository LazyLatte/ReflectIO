import {forwardRef, useImperativeHandle, useState, FC} from 'react';
...
const LevelClearModal = (props, ref) => {
  ...
  const toNextLevel = () => {
    (levelIdx+1 < AllLevels[difficulty].length) && navigate(`/play/${difficulty}/${levelIdx+2}`, {state: {difficulty, levelIdx: levelIdx+1}, replace: true});
    closeModal();
  }
          ...
          >
            ...
            >
                ...
                <Box position='absolute' left={0} right={0} top='125px' display='flex' flexDirection='row' justifyContent='space-around' alignItems='center'>
                  ...
                  <Button sx={styles.btn} disabled={levelIdx+1 >= AllLevels[difficulty].length} onClick={toNextLevel}>
